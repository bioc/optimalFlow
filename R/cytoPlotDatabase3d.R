...
#'
#' @return A three dimensional plot of ellipsoids containing the 95% of probability for the respectives components of the mixture distributions.
#' @examples
...
#' @import rgl
#' @export
cytoPlotDatabase3d <- function (database.cytometries.as.mixtures, dimensions = c(1, 2, 3), xlim = c(0, 8000),
                                ylim = c(0, 8000), zlim = c(0, 8000), xlab = "", ylab = "", zlab = "", colour = TRUE){
  ...
  if (colour) {
    ...
    for (j in 2:n.cells){
      rgl::plot3d(rgl::ellipse3d(cytometry.as.mixture[[j]]$cov[dimensions, dimensions],
                                 centre = cytometry.as.mixture[[j]]$
                                   mean[dimensions]), alpha = 0.5, add = TRUE, col = 1)
    }
    for (i in 2:n.cytometries){
      cytometry.as.mixture <- database.cytometries.as.mixtures[[i]]
      n.cells <- length(cytometry.as.mixture)
